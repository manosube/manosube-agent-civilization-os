...
Phase 7 structural-review round 9).
...
def _iter_installed_modules() -> list[Any]:
    ...
    for info in pkgutil.walk_packages(package.__path__, prefix=_PACKAGE_NAME + "."):
        # A module that fails to import is not a second canonical owner of anything -- it
        # contributes nothing to the inventory either way, and this scan's own fail-closed
        # behavior lives in whether the *expected* owner is still found exactly once, not in
        # tolerating every module in the tree importing cleanly.
...
def kernel_topology_inventory() -> dict[str, list[str]]:
    ...
    ``evaluate_closure`` (Kernel entry point / Closure Evaluation authority), every concrete
    class implementing the full canonical-State surface (``commit``/``load_current``/
    ``reconstruct``/``resolve_record`` -- write path, reconstruction source and persistence
    owner together), every ``evaluate_authority`` (Authority owner), and every
    ``commit_reflow`` (Reflow transition owner) actually found in the installed package.
    Exactly one of each is this vertical's own claim; more than one is a real, reproducible
    violation, not a proxy for one.
    """
...
def k001_single_kernel_entry_point() -> bool:
    ...
    exists anywhere in the installed package -- the sole Closure Evaluation authority /
    Kernel entry point this vertical exposes, not merely the one this call happened to
    import."""

    return len(kernel_topology_inventory()["closure_producers"]) == 1
...
def k002_single_canonical_state_owner() -> bool:
    ...
    package implements the full canonical-State surface (write path, reconstruction source
    and persistence owner together) -- no parallel State owner exists to consult instead."""

    return len(kernel_topology_inventory()["state_stores"]) == 1


def k003_single_authority_and_transition_owner() -> bool:
    """K-003 (Authority/transition/persistence ownership), its Static half: exactly one
    ``evaluate_authority`` Authority producer, exactly one ``commit_reflow`` Reflow
    transition committer, and the same single canonical State owner K-002 already proved --
    no second, competing canonical authority for any of the three roles this Invariant
    names."""
...
def r001_single_atomic_committer() -> bool:
    ...
    already proved is the one and only place an atomic commit can happen through -- there is
    no second commit path a caller could reach instead."""

    return k002_single_canonical_state_owner()
...
def r002_single_lineage_owner() -> bool:
    ...
    K-002's own inventory already required that owner to implement)."""
