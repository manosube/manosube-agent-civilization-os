"""Generate and locally sign one genuine Phase 21 independent reproduction submission (Issue #89,
PR #90 Round 5, `ADOPT_P90_R5_REAL_AGENT_FINAL_PROTOCOL_AND_EXECUTABLE_REPRODUCTION`, P90-R5-F3).

Runs entirely on the machine that holds the real private key (e.g. SHUKOU's own Windows PC). It
never requests, accepts, prints, logs, or persists raw private-key hex, and never touches the
private key at all except to open the caller-supplied encrypted PKCS8 PEM file in memory for the
one `sign()` call -- the key material is never written to this script's own output, and the PEM
path/passphrase are never included in `--output`.

Every field of the emitted submission is produced mechanically by this package's own shipped
identity/engine functions -- `independent_reproduction_submission_id`,
`..._semantic_fingerprint`, and `independent_reproduction_submission_signing_payload` -- never
hand-authored. Before ever writing the completed submission to `--output`, this script verifies
the signature itself, locally, against the already-registered public key
(`verify_ed25519_signature`) -- a submission this script itself cannot verify is never written.

One end-to-end entrypoint: `scripts/reproduce_and_sign_comparative_benchmark_submission.ps1`
wraps this script for PowerShell/Windows."""
...
#: reproducer (PR #90 comment 5709021178, author `manosube`, `OWNER`) -- this script only ever
#: reads this constant to confirm the caller's own PEM matches it; it is never used to derive or
...
def _reproduce_raw_events() -> list[dict[str, Any]]:
    """Mechanically reproduce the frozen corpus by running this repository's own shipped,
    tested reproduction procedure -- never hand-authored raw events."""

    sys.path.insert(0, str(ROOT))
    from tests.comparative_benchmark.orchestrator import run_comparative_benchmark

    with tempfile.TemporaryDirectory(prefix="cb21-independent-reproduction-") as tmp:
        result = run_comparative_benchmark(Path(tmp))
    return list(result["reproduced_raw_events"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(  # noqa: T201 -- this script's own CLI progress report, to stderr
        "Reproducing the frozen corpus (this repository's own shipped procedure)...",
        file=sys.stderr,
    )
