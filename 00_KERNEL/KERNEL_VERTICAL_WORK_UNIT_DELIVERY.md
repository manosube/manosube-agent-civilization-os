...
The acceptance evidence MUST record both lineage identities, the reviewed base-state identity, the reviewed proposed-state identity, the exact Change boundary, the change-review / acceptance-surface receipt identity, the receipt's immutable revision identity or SHA-256 of its canonical content, and the independence result. For a text receipt, canonical content is the exact source body returned by the acceptance capability after converting each CRLF byte pair (`0x0D 0x0A`) to one LF byte (`0x0A`), preserving every other byte—including any lone CR (`0x0D`), trailing whitespace and terminal-newline presence—and then encoding the resulting Unicode text as UTF-8. Evidence is invalid if the base, proposed state, Change boundary, receipt identity, receipt revision, or canonical receipt content digest differs. A mutable URL or object ID without a bound revision or canonical content digest is insufficient. If distinct lineage or either state identity cannot be evidenced, acceptance fails closed.
