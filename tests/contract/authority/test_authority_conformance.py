...
# --------------------------------------------------------------------------- #
# 19. Scope normalization has exactly one owner
# --------------------------------------------------------------------------- #


def test_scope_normalization_has_one_owner() -> None:
    """``CANONICAL_SCOPE_NORMALIZATION_OWNER_COUNT=1``, read from the package.

    ``paths`` and ``subjects`` are sets written as lists, and their canonical form is decided
    in ``authority.scope.canonical_scope`` and nowhere else. A second sort would be a second
    answer to what the canonical form is, and the first time the two disagreed the
    disagreement would be silent -- the shape ADR-0027 and ADR-0028 record four times over.
    """

    from pathlib import Path

    package = Path(__file__).resolve().parents[3] / "src" / "manosube_agent_civilization"
    owner = package / "authority" / "scope.py"

    definitions = [
        module
        for module in sorted(package.rglob("*.py"))
        if "def canonical_scope(" in module.read_text(encoding="utf-8")
    ]
    assert definitions == [owner], definitions

    # Nobody else sorts a scope collection for themselves.
    for module in sorted(package.rglob("*.py")):
        if module == owner:
            continue
        source = module.read_text(encoding="utf-8")
        for forbidden in ('sorted(scope[', 'sorted(scope.get(', '["paths"])', "['paths'])"):
            assert f"sorted({forbidden}" not in source, (module, forbidden)


def test_the_identity_owner_reuses_the_normalizer_rather_than_sorting() -> None:
    ...
    identity = (
        ...
        / "src" / "manosube_agent_civilization" / "authority" / "identity.py"
    ).read_text(encoding="utf-8")
    assert "canonical_scope(scope)" in identity
    assert "sorted(" not in identity


def test_change_identity_does_not_normalize_for_itself() -> None:
    ...
    change_identity = (
        Path(__file__).resolve().parents[3]
        / "src" / "manosube_agent_civilization" / "change" / "identity.py"
    ).read_text(encoding="utf-8")
    assert "sorted(" not in change_identity
    assert "canonical_scope" not in change_identity
