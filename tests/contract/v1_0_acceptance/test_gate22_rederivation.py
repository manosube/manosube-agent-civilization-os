...
subprocesses, and the Deferred Differences register is actually read to derive the
twelfth (`ALL_V1_0_BLOCKING_DIFFERENCES_CLOSED`) -- proving the whole v1.0 acceptance
bundle assembles end-to-end against the live repository, not a fixture.
"""
...
def test_full_bundle_assembles_against_the_live_repository() -> None:
    ...
    # All twelve predicates are owned by already-accepted, currently-passing test suites
    # or by this repository's own register state -- they must mechanically rederive PASS,
    # never FAIL or UNKNOWN, proving this repository's own accepted evidence still holds
    # at this head.
    for predicate, row in bundle["gate_22_predicate_matrix"].items():
        assert row["verification_result"] == "PASS", (predicate, row)

    # ALL_V1_0_BLOCKING_DIFFERENCES_CLOSED is honestly PASS at this post-merge
    # source-sync head: SHUKOU adopted an explicit disposition for four of the five
    # previously-open records (`ADOPT_P92_V1_0_DIFFERENCE_DISPOSITION_R1_AND_FINAL_GATE22_SYNC`,
    # Issue #92 comment `5755827293`); PR #94 Structural Review Round 1 (`P94-R1-F1`,
    # adopted `ADOPT_P94_R1_F1`, comment `5760099935`) correctly held FD-0002 open until
    # its own adopted closure condition -- the README correction existing on accepted
    # `main` plus post-merge after-state re-observation -- was genuinely satisfied. PR #94
    # merged (`ddf906eb7cefa43a0c43ce3dc1c208c7c5602439`), the Structural Advisor
    # independently re-observed the resulting `main`
    # (`P94_POST_MERGE_AFTER_STATE_R1`, Issue #92 comment `5768458110`), and SHUKOU adopted
    # FD-0002's closure on that evidence (`ADOPT_P92_FINAL_POST_MERGE_SOURCE_SYNC_R1`,
    # comment `5768466436`). All seven active records (DD-0001, DD-0002, DC-0001, FD-0001,
    # FD-0002, FD-0003, FD-0005) are still present -- records are never deleted -- and all
    # now read NON_BLOCKING (see blocking_differences.py).
