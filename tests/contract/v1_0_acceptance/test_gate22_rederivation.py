...
def test_full_bundle_assembles_against_the_live_repository() -> None:
    ...
    # test suites from Phases 0-21 -- they must mechanically rederive PASS, never FAIL or
    # UNKNOWN, proving this repository's own accepted evidence still holds at this head.
    ...
    # ALL_V1_0_BLOCKING_DIFFERENCES_CLOSED is honestly UNKNOWN at this round -- several
    # Deferred Differences records require an explicit SHUKOU disposition this package must
    # never infer on its own authority (see blocking_differences.py). A silent PASS here
    # would be exactly the kind of over-claim this Gate exists to prevent.
