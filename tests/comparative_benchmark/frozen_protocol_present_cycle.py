...
comment 5715652626) -- the ``MANOSUBE_PRESENT`` condition composer for the frozen-protocol
two-task real-Agent corpus (``examples/comparative_benchmark/real_agent_corpus/
frozen_protocol/``), run genuinely after this round's own protocol freeze is durably committed.

Adapted from ``tests/comparative_benchmark/real_agent_present_cycle.py`` (the exploratory
Round 6 composer, comment 5714630885), kept as its own independent module -- pointed at
:mod:`tests.fixtures.comparative_benchmark_frozen_protocol` rather than :mod:`tests.fixtures.
comparative_benchmark_real_agent` -- rather than importing/parametrizing it directly, the same
``PHASE_8_FIXTURE_BINDING_NE_PHASE_20_FIXTURE_BINDING=true`` convention both of that module's
and ``tests/natural_cycle/proof.py``'s own docstrings already establish for keeping each phase/
round's composer independently kept, and never mutating the exploratory fixture pair the
Structural Advisor's governance correction requires be retained untouched.
...
from tests.fixtures import comparative_benchmark_frozen_protocol as ra
